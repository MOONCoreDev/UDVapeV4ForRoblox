...
vape:Clean(replicatedStorage.RoundTimer:GetAttributeChangedSignal('TimeLeft'):Connect(function()
	if replicatedStorage.RoundTimer:GetAttribute('TimeLeft') ~= '' and replicatedStorage.RoundTimer:GetAttribute('TimeLeft') ~= -1 then
...
for i,v in {'FOV', 'MurderMystery', 'Chat', 'Invisible', 'AntiRagdoll'} do
	vape:Remove(v)
end
